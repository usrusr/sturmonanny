...
//  this : Lg with Logging[Lg] =>
  ...
  def error(msg: => AnyRef, t: => Throwable): Unit = logger.error(t,msg.toString)

 

  def fatal(msg: AnyRef): Unit = logger.fatal(msg.toString)
  def fatal(msg: AnyRef, t: Throwable): Unit = logger.fatal(t,msg.toString)
  ...
  def info(msg: => AnyRef, t: => Throwable): Unit = logger.info(t,msg.toString) 

  def isWarnEnabled: Boolean = logger.isLoggable(Logger.WARNING)
  def warn(msg: => AnyRef): Unit = logger.warning(msg.toString)
  def warn(msg: => AnyRef, t: => Throwable): Unit = logger.warning(t,msg.toString)
