...
class Pilots extends Domain[Pilots] with actor.LiftActor with NonUpdatingMember with Logging {
  ...
  def roughPlayerCountRedBlue = {
    ...
    if (playerCountSince + 10000 < server.time.now) {
      ...
      playerCountStateRedBlue = items.values.foldLeft((0, 0)) { (redBlue, pilot) =>
      ...
      msg match {
        ...
        case Commands.helpCommand(cmd) => (if (cmd == null) "" else cmd.trim) match {
          ...
          case "recruit" => {
            ...
            chat("you can recruit for up to " + conf.recruiting.time.apply + " s after starting", 1000)
            val planeDesc = if (state.planeName != null && !state.planeName.isEmpty) "a " + state.planeName else "the same plane as you"
            chat("the invitation will be accepted if the other pilot takes " + planeDesc + " within that timespan", 2000)
            chat("death-timeout is skipped for your recruits", 3000)
            val percs = server.rules.recruiterPercents
            chat("you will pay " + percs + "% of your recruit's takeoff fee and get " + percs + "% of his landing refund", 4000)
          }
