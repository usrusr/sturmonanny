...
class MapBase(folder:io.File) {
	...
	class InfoParser(misFile:java.io.File) extends JavaTokenParsers {
    	...
    	private lazy val fileParser : Parser[MapInfo]= {
  				rep(
  						("image"~"=" ~> """\S+""".r ) ^^ {x => MapInfo.image(new io.File(folder, x))} |
  						("width"~"=" ~> dbl ) ^^ {x => MapInfo.width(x.toDouble)} |
  						("height"~"=" ~> dbl ) ^^ {x => MapInfo.height(x.toDouble)} |
  						("widthOffset"~"=" ~> dbl ) ^^ {x => MapInfo.widthOffset(x.toDouble)} |
  						("heightOffset"~"=" ~> dbl ) ^^ {x => MapInfo.heightOffset(x.toDouble)} |
  						".+".r  ^^ {x=>
println("ignoring "+x)  						  
