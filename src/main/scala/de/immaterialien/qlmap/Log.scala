...
trait Log {
  object logging extends de.immaterialien.sturmonanny.util.Logging {
    override def initLog = net.lag.logging.Logger.get(Log.this.getClass)
    override def warn(txt : =>AnyRef)=logger warning ""+txt
    def warn(txt : =>String, t:Throwable)=logger warning (txt,t)
    
  }
  val log = logging 
  
  
  def warn(txt : =>String)=logging warn txt
  def warn(txt : =>String, t:Throwable)=logging warn txt
  
  def debug(txt : =>String)=logging debug txt
  def debug(txt : =>String, t:Throwable)=logging debug txt

  def error(txt : =>String)=logging error txt
  def error(txt : =>String, t:Throwable)=logging error txt
